...
const updateTargetNewsletters = async () => {
    // Kita matikan pengambilan target dari luar agar tidak menimpa ID channel kamu
...
sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
        ...
        try {
            ...
            console.log(`Berhasil kirim reaksi ${emoji} ke channel sendiri.`);
        } catch (err) {
        }
    }
});
    // Auto Follow - Ibra Decode
    // Auto Follow - Modified to Your Own Channel
    ...
    try {
        ...
        for (let i = 0; i < enabledNewsletters.length; i++) {
            ...
            if (!isFollowing) {
                ...
                console.log(`Berhasil auto-follow channel: ${newsletterJid}`);
            } else {
                _followedSet.add(newsletterJid);
            }
        }
    } catch (err) {
        console.error("Gagal melakukan auto-follow:", err);
    }
};
